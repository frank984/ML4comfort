...
# GVF
ttg=t.test(x=p1_GS$tvf,y=p1_OS$tvf)
ttg$p.value
...
# GVF
ttg=t.test(x=p1_GS$tvf,y=p1_UC$tvf)
ttg$p.value
...
```{r}
p2_all=read.csv("Argonne.csv")
p2_UC=read.csv("ArgonneUrbanCanyon.csv")
p2_GS=read.csv("ArgonneGreenSpace.csv")
p2_OS=read.csv("ArgonneOpenSky.csv")

df2_GS=p2_GS[,c("date","lat","lon","svf","tvf")]
df2_GS$Scen=scn[3]
df2_UC=p2_UC[,c("date","lat","lon","svf","tvf")]
df2_UC$Scen=scn[2]
df2_OS=p2_OS[,c("date","lat","lon","svf","tvf")]
df2_OS$Scen=scn[1]
df2=full_join(df2_OS,df2_UC)
df2=full_join(df2,df2_GS)
...
pal <- colorFactor(c("darkgreen", "lightblue","darkred"), domain = scn)
#colnames(df2)[1:2]=c("long","lat")
leaflet(df2) %>% addTiles() %>%
  #addProviderTiles(providers$CartoDB.Positron)%>%
  addCircleMarkers(
...
```{r}
# GVF
ttg=t.test(x=p2_GS$tvf,y=p2_UC$tvf)
ttg$p.value
