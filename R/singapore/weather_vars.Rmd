...
```{r}
# (*)In air_temp, consider only times at which we have survey measurement
...
x_shr=air_temp1_sp.full_shrink
...
spat_kr=function(x_shr,var_name,grid_coords){
  ...
  for(i in 1:nrow(x_shr)){
    ...
    v.fit <- fit.variogram(v.vgm, vgm(10,"Gau",10,0))
...
start=Sys.time()
prv=spat_kr(air_temp1_sp.full_shrink,"air_temp",grid_coords)
end=Sys.time()
end-start
```
...
match_krg_weath_coz=function(df_cozie,x_kriged,var_name){
  ...
  # match times
  AB=bayesbio::nearestTime(x_kriged,df_cozie,"time","time")
  AB$newvar=0
...
```{r}
st=Sys.time()
prvmatch=match_krg_weath_coz(df_cozie,x_kriged,var_name)
######Errore in match_krg_weath_coz(df_cozie, x_kriged, var_name) : 
#####oggetto 'x' non trovato
en=Sys.time()
en-st
```
