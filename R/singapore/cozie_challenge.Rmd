...
1) What range of values should we employ for latitude and longitude? Opting for the maximum and minimum values recorded by the Cozie app or the weather stations.

2) What level of granularity should we adopt for latitude and longitude? I don't have a definitive answer to this query. Undoubtedly, higher granularity implies increased computational expenses.
...
indx_air=locations$id%in%colnames(air_temp1_sp.full)

# spatial locations of weather stations in singapore
singapore_sp_air =locations[indx_air,c("latitude","longitude")]
row.names(singapore_sp_air) = locations$id[indx_air]
singapore_sp_air = SpatialPoints(singapore_sp_air)

# Define time points
time=air_temp1_sp.full$time
...
air_t=as.vector(t(air_temp1_sp.full[,-1]))
airdata=data.frame(air=air_t,time=rep(time,each=ncol(air_temp1_sp.full[,-1])))

stfdf_air = STFDF(singapore_sp_air, time, data = airdata)
...
singapore_gridded <- SpatialPoints(cbind(rep(grid_lat,length(grid_lat)), 
